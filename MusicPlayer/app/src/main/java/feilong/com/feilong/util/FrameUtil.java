...
public class FrameUtil {
	...
	public static Frame CalcFrame(String fileName, int fileSize) throws Exception {
		...
		if (head.equals("ID3")) {
			...
			System.out.println("ID3shijia" + ID3V2_frame_size);
			if ((header[5] & 0x40) == 64) {
				fis.read(header);
			}
			int seek = 0;
			...
			do {
				...
				System.out.println(seek + "wwwww " + new String(header, 0, 4));
				...
				if (new String(header, 0, 4).equals("APIC") && (seek > 14)) {
					...
					for (i = 0; i < seek; i++) {
						...
						if (buf[0] == he[0]) {
							...
							if (counts == 1) {
								System.out.println("   wwww finals" + i);
								counts = 2;
								break;
							} else {
								counts = 0;
		...
		if (vbrHead.toLowerCase().equals("xing") || vbrHead.toLowerCase().equals("info")) {
			...
			System.out.println(frameSizeArray[0] + " " + frameSizeArray[1] + " " + frameSizeArray[2] + " " + frameSizeArray[3]);
			frame.CalcFrameSize();
			System.out.println("  wwwwww" + frame.frameCount);
		} else {
			System.out.println("没有INFO");
		...
		frame.playTime = secs;
		System.out.println((int) (secs / 3600) + " " + (int) (secs % 3600 / 60) + " " + (int) (secs % 3600 % 60));
		return frame;
	}
